...
class db_class {
   ...
   function connect($host='', $user='', $pw='', $db='', $new_db=FALSE, $persistant=true, $select_db = true) {
      ...
      // Select the database
	  if($select_db)
		if (!$this->select_db($db,$new_db)) return false;
   ...
   function check_db($db='',$create=true)
   {
   ...
   function delete($table, $where = array(), $like = array(), $limit = FALSE, $isIn = array())
	{
		if(count($where) >=1)
		{		
		...
		$sql .= ($isIn != '' AND count($isIn) >=1) ? " WHERE ". $isField : '';
		
		$limit = ( ! $limit) ? '' : ' LIMIT '.$limit;
	
		//$sql = "DELETE FROM ".$table.$conditions.$limit;
		return $this->odelete($sql);
	}
	
    function odelete($sql) {
      ...
      //$this->row_count = mysql_affected_rows($r);
      return $r;
   }
   
   function get_next_ai($tbname) {

      $r = mysql_query("SHOW TABLE STATUS LIKE '".$tbname."'");
      if (!$r) {
         $this->last_error = mysql_error();
         return  mysql_error();
      }
   ...
   function insert_sql($sql) {
	...
	function get($table, $fields = '', $where, $orderby = array(), $limit = FALSE)
	{		
   ...
   {	
   		foreach($values as $key => $val)
		{
			...
			if ($val != 'NULL' AND $val != 'CURRENT_TIMESTAMP'){
				$vals[] = "'".$val."'";
			}else{
				$vals[] = $val;
			}
